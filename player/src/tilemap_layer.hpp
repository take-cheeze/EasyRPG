...
private:
	Bitmap* chipset;
	boost::scoped_ptr<BitmapScreen> chipset_screen;
	...
	std::auto_ptr<BitmapScreen> GenerateAutotiles(int count, const std::map<uint32, TileXY>& map);

	TileXY GetCachedAutotileAB(short ID, short animID);
	TileXY GetCachedAutotileD(short ID);

	std::auto_ptr<BitmapScreen> autotiles_ab_screen;
	std::auto_ptr<BitmapScreen> autotiles_d_screen;
