...
template class BitmapUtils<format_B8G8R8A8>;
...
struct blit2x_traits<PFsrc, PFdst, false> {
	static void copy_pixel_2x(uint8*& dst_pixels, const uint8*& src_pixels) {
		uint8 r, g, b, a;
		PFsrc::get_rgba(src_pixels, r, g, b, a);
		src_pixels += PFsrc::bytes;
		PFdst::set_rgba(dst_pixels, r, g, b, a);
...
class Blit2X {
	static const bool same =
		...
		PFsrc::rbits 		== PFdst::rbits  &&
		PFsrc::rshift		== PFdst::rshift &&
		PFsrc::gbits 		== PFdst::gbits  &&
		PFsrc::gshift		== PFdst::gshift &&
		PFsrc::bbits 		== PFdst::bbits  &&
		PFsrc::bshift		== PFdst::bshift &&
		PFsrc::abits 		== PFdst::abits  &&
		PFsrc::ashift		== PFdst::ashift &&
...
public:
	static void Blit(uint8* dst_pixels, int dst_pitch, Bitmap* src) {
		src->Lock();
		...
		for (int y = 0; y < src_height; y++) {
			for (int x = 0; x < src_width; x++) {
				blit2x_traits_type::copy_pixel_2x(dst_pixels, src_pixels);
...
template class Blit2X<format_B8G8R8A8, PixelFormat<32,false,true,false, true, 8,16,8,8,8,0,8,24> >;
