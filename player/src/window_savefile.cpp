...
void Window_SaveFile::Refresh() {
	...
	contents->TextDraw(8, 32+2, 1, Data::database.terms.lvl_short);

	int lx = Surface::GetTextSize(Data::database.terms.lvl_short).width;
	out.str("");
	out << std::setw(2) << std::setfill(' ') << actor->GetLevel();
	contents->TextDraw(8+lx, 32+2, Font::ColorDefault, out.str());

	contents->TextDraw(42, 32+2, 1, Data::database.terms.hp_short);

	int hx = Surface::GetTextSize(Data::database.terms.hp_short).width;
	out.str("");
	out << actor->GetHp();
