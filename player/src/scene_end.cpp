...
void Scene_End::CreateCommandWindow() {
	...
	options.push_back(Data::database.terms.yes);
	options.push_back(Data::database.terms.no);
...
void Scene_End::CreateHelpWindow() {
	int text_size = Surface::GetTextSize(Data::database.terms.exit_game_message).width;

	help_window = new Window_Help(160 - (text_size + 16)/ 2,
		72, text_size + 16, 32);
	help_window->SetText(Data::database.terms.exit_game_message);
